...
// Define interface for mock contract with new functionality
#[starknet::interface]
pub trait IMockFiatToken<TContractState> {
    // Original FiatToken functions
    ...
    // New mock functions for testing upgrade
    fn get_version_v2(self: @TContractState) -> u256;
    fn mock_storage_value(self: @TContractState) -> u256;
    fn set_mock_storage_value(ref self: TContractState, value: u256);
}

// Mock contract that extends FiatToken functionality
#[starknet::contract]
pub mod MockFiatToken {
    ...
    // Import errors from fiat_token module
    use stablecoin::fiat_token::errors::Errors;
    // Import events from fiat_token module
    use stablecoin::fiat_token::events::{Approval, Burn, FiatTokenInitialized, Mint, Transfer};
    ...
    pub enum Event {
        ...
        Transfer: Transfer,
        Approval: Approval,
        // New event for mock functionality
        MockValueUpdated: MockValueUpdated,
    }

    #[derive(Drop, starknet::Event)]
    pub struct MockValueUpdated {
        #[key]
    ...
    fn constructor(
        ...
        // Initialize mock storage with default value
        self.mock_storage_value.write(42);
    }

    #[abi(embed_v0)]
    impl MockFiatTokenImpl of super::IMockFiatToken<ContractState> {
        ...
        // New functions for mock contract
        fn get_version_v2(self: @ContractState) -> u256 {
            2 // Version 2 to indicate this is the upgraded contract
        }

        fn mock_storage_value(self: @ContractState) -> u256 {
            self.mock_storage_value.read()
        }

        fn set_mock_storage_value(ref self: ContractState, value: u256) {
            self.pausable.assert_not_paused();
            self.blocklistable.assert_not_blocklisted(get_caller_address());
            let old_value = self.mock_storage_value.read();
            self.mock_storage_value.write(value);

            self
                .emit(
                    MockValueUpdated { caller: get_caller_address(), old_value, new_value: value },
                );
        }
    }
